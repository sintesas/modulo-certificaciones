		...
		<form method="post" action="{{ route('download') }}">
			...
			<label for="recipient-name" class="col-form-label">Tipo Nómina:</label>				
				...
				@foreach($tiposnomina as $tipo)
					...
					@if($tipo->tipo_nomina == 2)
					<option value="{{$tipo->tipo_nomina}}">PRIMA DE SERVICIO</option>
					@elseif($tipo->tipo_nomina == 3)
					<option value="{{$tipo->tipo_nomina}}">PRIMA DE NAVIDAD</option>
					@elseif($tipo->tipo_nomina == 4)
					<option value="{{$tipo->tipo_nomina}}">RETROACTIVO</option>
					@elseif($tipo->tipo_nomina == 5)
					<option value="{{$tipo->tipo_nomina}}">ADICIONAL</option>
					@elseif($tipo->tipo_nomina == 6)
					<option value="{{$tipo->tipo_nomina}}">ADICIONAL VIGENCIA ACTUAL</option>
					@elseif($tipo->tipo_nomina == 1)
					<option value="{{$tipo->tipo_nomina}}">NOMINA MENSUAL</option>
					@else
					<option value="{{$tipo->tipo_nomina}}">ADICIONAL VIGENCIA EXPIRADA</option>
					@endif					
				@endforeach					
				</select>	
			</div>			
